...
const AddCompras = ({ isOpen, onClose, clients, clientID, updateClient }) => {
  ...
  const updatedData = {
    purchaseCount: (currentClient.purchaseCount || 0) + parseInt(clientPurchases)
  };

  try {
    await updateClient(clientID, updatedData);
    notifySuccess("Compras adicionadas com sucesso!");
    handleClose();
  } catch (error) {
